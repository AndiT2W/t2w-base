...
export function FolderLink({ label, href, available, children, icon }: { label: string; href: string | null; available: boolean; children: ReactNode; icon?: "outlook" | "sharepoint" }) {
  ...
  if (!available || !href) return <span className="inline-flex items-center text-muted-foreground" title={`${label}: nicht verknüpft`} aria-label={`${label}: nicht verknüpft`}><ServiceIcon className="size-4" /></span>;
  return <span className="flex min-w-0 items-center gap-1.5" onClick={(event) => event.stopPropagation()}>
    <a href={href} target="_blank" rel="noreferrer" className="inline-flex min-w-0 items-center gap-1 truncate text-primary hover:underline" title={href}>
      <ServiceIcon className="size-4 shrink-0" /><span className="truncate">{children}</span><ExternalLink className="size-3 shrink-0" />
    </a>

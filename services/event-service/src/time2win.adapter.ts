...
export class HttpTime2winAdapter implements Time2winAdapter {
  private readonly baseUrl = (
    process.env.TIME2WIN_API_BASE_URL ?? "https://time2win.at/api/1.1"
  ).replace(/\/+$/, "");
    ...
    const raceSnapshots = await Promise.all(
      races.map(async (race) => {
        ...
        const stats = await this.request(
          `/participants/${eventId}/stats${id === null ? "" : `?race_id=${id}`}`,
        );
        return {
          id: id ?? 0,
          name:
            firstString(data, ["name", "race_name", "raceName", "title"]) ??
            `Bewerb ${id ?? ""}`.trim(),
          participantCount: participantCount(stats),
        };
      }),
    );
    return {
      eventId,
      name:
        firstString(event, ["name", "event_name", "eventName"]) ??
        firstString(source, ["name", "event_name", "eventName"]),
      sportName:
...
const firstString = (source: Record<string, unknown>, keys: string[]) =>
  keys.map((key) => string(source[key])).find((value): value is string => value !== null) ?? null;
const firstNumber = (source: Record<string, unknown>, keys: string[]) =>
  keys.map((key) => number(source[key])).find((value): value is number => value !== null) ?? null;

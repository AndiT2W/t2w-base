...
export class SettingsController {
  constructor(
    ...
    private readonly outlook: OutlookFolderService,
  ) {}
  @Get() get() {
  ...
  async outlookStatus() {
    ...
    try {
      return await this.outlook.checkConnection(settings.outlookMailbox);
    } catch (error) {
      return {
        connected: false,
        reason: error instanceof Error ? error.message : "OUTLOOK_CONNECTION_FAILED",
      };
    }
  }
}

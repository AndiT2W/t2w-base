...
export class MicrosoftGraphClient implements OutlookGraphClient {
  ...
  private async accessToken(): Promise<string> {
    ...
    if (!response.ok) throw new OutlookGraphError(response.status, "OUTLOOK_GRAPH_TOKEN_REQUEST_FAILED", response.headers.get("retry-after") ?? undefined);
  ...
  private async request<T>(url: string, init?: RequestInit): Promise<T> {
    ...
    const response = await fetch(url.startsWith("http") ? url : `${this.baseUrl}${url}`, { ...init, headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", ...init?.headers } });
    if (!response.ok) throw new OutlookGraphError(response.status, `OUTLOOK_GRAPH_${response.status}`, response.headers.get("retry-after") ?? undefined);
    return response.json() as Promise<T>;
  }

  async listChildFolders(mailbox: string, parentId: string): Promise<OutlookFolder[]> {
    const folders: OutlookFolder[] = [];
    let next: string | undefined = `/users/${encodeURIComponent(mailbox)}/mailFolders/${encodeURIComponent(parentId)}/childFolders?$top=1000`;
    while (next) {
      const result: { value: OutlookFolder[]; "@odata.nextLink"?: string } = await this.request<{ value: OutlookFolder[]; "@odata.nextLink"?: string }>(next);

...
@Module({
  providers: [
    ...
    {
      provide: OutlookFolderService,
      useFactory: (prisma: PrismaService, graph: MicrosoftGraphClient) =>
        new OutlookFolderService(prisma, graph),
      inject: [PrismaService, "OUTLOOK_GRAPH_CLIENT"],
    },
    {
      provide: EventCommunicationHub,
      useFactory: (repository: PrismaEventCommunicationAdapter, graph: MicrosoftGraphClient) =>
        new EventCommunicationHub(repository, graph),

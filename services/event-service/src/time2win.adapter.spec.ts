...
describe("HttpTime2winAdapter", () => {
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            event_name: "OstseeMan 2027",
            type_name: "Triathlon",
            races: [{ race_id: 7709, race_name: "Langdistanz" }],
          }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ races: [{ race_id: 7709, participants_count: 300 }] }),
        }),
    );
  ...
  it("fails before transport when authentication is missing", async () => {
    ...
    await expect(new HttpTime2winAdapter().snapshot(42)).rejects.toThrow(
      "TIME2WIN_API_KEY_NOT_CONFIGURED",
    );
  });
  ...
  it("uses the legacy API endpoint when no base URL is configured", async () => {
    ...
    const fetch = vi
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          type_name: "Run",
          races: [{ race_id: 7, race_name: "Main race" }],
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ races: [{ race_id: 7, participants_count: 120 }] }),
      });
    ...
    expect(fetch.mock.calls).toEqual([
      [
        "https://time2win.at/api/1.1/event/42/eventdata",
        { headers: { Authorization: "Bearer test-key" } },
      ],
      [
        "https://time2win.at/api/1.1/participants/42/stats?race_id=7",
        { headers: { Authorization: "Bearer test-key" } },
      ],
    ]);
  });
  ...
  it("uses the configured Data Hub endpoint for synchronization", async () => {
    ...
    const fetch = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          event_name: "Data Hub event",
          type_name: "Triathlon",
          races: [{ race_id: 8, race_name: "Sprint" }],
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ races: [{ race_id: 8, participants_count: 240 }] }),
      });
    vi.stubGlobal("fetch", fetch);
    ...
    expect(fetch.mock.calls).toEqual([
      [
        "https://data-hub.time2win.at/event/42/eventdata",
        { headers: { Authorization: "Bearer test-key" } },
      ],
      [
        "https://data-hub.time2win.at/participants/42/stats?race_id=8",
        { headers: { Authorization: "Bearer test-key" } },
      ],
    ]);
  });

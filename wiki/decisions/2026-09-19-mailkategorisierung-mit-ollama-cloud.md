...
Die manuelle Prüfung ist im Eventdetail unter **Kommunikation → Mail analysieren** als Sheet erreichbar. Das aktuelle Event wird als Kontext angezeigt, die Testanfrage vergleicht aber standardmäßig alle aktiven Events. So lässt sich die Eventfindung testen, ohne bereits eine Zuordnung oder Outlook-Aktion auszulösen.

Zusätzlich gibt es **Präfixe anwenden** für die synchronisierten Mails des aktuellen Events. Nach Bestätigung wird ausschließlich bei einem exakten Treffer mit `eventConfidence === 1` der Outlook-Betreff per Graph auf `[Eventname] Originalbetreff` gesetzt. Unsichere Treffer bleiben unverändert; die Änderung wird lokal und im Auditlog festgehalten. Der Browserablauf ist in [`mail-classifier.spec.ts`](../../tests/e2e/mail-classifier.spec.ts) abgesichert.

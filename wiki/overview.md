...
- Auszahlungen verwenden fachlich ein einzelnes Lebenszyklusfeld `status`; Mailversand und tatsächliche Zahlung bleiben getrennte Meilensteine. Automations-/Retry-Metadaten liegen in `AutomationClaim`, und die Empfängeradresse wird im Auszahlungssnapshot festgehalten. Die Schemaänderung ist als Migration vorbereitet; kein ClickUp-Produktivimport wurde ausgeführt. Siehe [Auszahlungsspezifikation](tasks/event-payouts-spec.md) und [ADR 0004](../docs/adr/0004-payout-single-lifecycle-status.md).

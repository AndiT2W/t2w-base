...
- Die Telefonansicht gegen das Artboard „Übersicht — Mobil" gebracht. **Kopfzeile:** auf Ansichten unter `md` trägt sie jetzt dunkel den Seitentitel und eine Lupe; das Suchfeld klappt erst auf Druck auf. Brotkrume und große Überschrift entfallen dort — vorher standen Suchfeld, Krume, Überschrift und Beschreibung übereinander, bevor die erste Zeile der Liste zu sehen war. Die Beschreibungszeile bleibt: sie führt Verweise wie den Veranstalter eines Events. Entschieden wird das im Bau über [`useSchmal`](../src/components/t2w/Seitenkopf.tsx), nicht mit `hidden` — sonst stünde der Titel zweimal im Baum und eine Vorlesehilfe fände zwei Überschriften. **Eventkarten:** Statuspunkt vor dem Namen statt Statuschip in eigener Zeile, Pfeil rechts, darunter Eventcode und Zeitraum, dann die Aufgabenlage als Chips („n überfällig", „n offen", „Ohne Ordner"); Veranstalter, Sportart, Services und die Ordnerverweise stehen als Fußzeile der Karte. Die ganze Karte ist der Weg ins Event. **Nicht übernommen:** der Entwurf zeigt auf dem Telefon nur zwei Kennzahlkacheln und drei Schnellfilter, die Anwendung hat vier und fünf plus Statusauswahl — das wäre Informationsverlust, nicht Gestaltung. Ebenso bleiben „Kalender" und „Event anlegen" im Seitenkopf, die der Entwurf auf dem Telefon nicht zeichnet. Die Modulleiste am unteren Rand stimmte bereits. Regression: [`mobile-and-finance.spec.ts`](../tests/e2e/mobile-and-finance.spec.ts) prüft Kopfzeile, Lupe und Kartenaufbau.

- Die vier offenen Artboard-Punkte umgesetzt. **Hardware:** Statuschips mit Anzahl (Alle · Offen · Überfällig · Zurück) statt Auswahlfeld und Überfällig-Schalter — die Anzahl ist die erste Frage der Seite und soll nicht erst nach dem Aufklappen zu sehen sein; dazu der Filter „Empfänger", der Knopf „Objekt anlegen" im Kopf (führt zu den Auswahllisten) und die Kacheln des Entwurfs (Aktuell ausgegeben · Rückgabe offen · Rückgabe überfällig · Diese Woche fällig). **Dabei gefunden und behoben:** der Filter „Ausgabeart" zählte in der Filteranzeige mit, wirkte aber nie auf die Liste. **Auszahlungen:** vier Kacheln (Offen · Zum Versand markiert · Ausbezahlt · Storniert) mit Betrag und Belegzahl statt der einen Kachel „Summe gefiltert"; sie zählen die Menge des Jahres- und Eventfilters, nicht die Statusauswahl darunter — sonst zeigte „Offen" nach einem Klick auf „Ausbezahlt" eine Null. **Einstellungen:** die fünf Bereiche stehen jetzt zusätzlich als Reiterleiste auf der Seite; vorher hingen sie nur im Untermenü der Seitenleiste, und wer die Seite über einen Link betrat, kam von dort nicht weiter. **Artboard:** „Sammelexport" ist aus dem Auszahlungsentwurf entfernt — am 21.09. gestrichen. Die Kennzahlreihe trägt jetzt einen Namen (`role="group"`), damit eine Vorlesehilfe sie als Block ansagen kann. Regression: Browsertests für die Lagechips samt Empfängerfilter, die vier Auszahlungskacheln und die Reiterleiste der Einstellungen.
...
- `src/styles_begin.css` gelöscht. Die Datei war kein Versehen, sondern der am 21.08.2026 bewusst abgelegte Stylesheet-Stand vor der CI-Umstellung — aber ein Rückweg, den die Git-Historie ohnehin verwahrt, und sie trug noch das `@custom-variant dark`, das am 21.09.2026 aus dem Projekt geflogen ist. Importiert wurde sie nie. Der archivierte Stand ist unter `git show 52bd4fad:src/styles_begin.css` erreichbar; die Verweise in [TIME2WIN CI und Branding](concepts/time2win-ci-branding.md) und [overview.md](overview.md) zeigen jetzt dorthin statt auf die Datei. Der Logeintrag vom 21.08.2026 bleibt unverändert stehen: er hält fest, was damals geschah.
...
Der Kommunikationsreiter kann jetzt synchronisierte E-Mails nach Bestätigung mit **Präfixe anwenden** markieren. Die serverseitige Aktion klassifiziert gegen alle aktiven Events und schreibt nur bei exakt passendem Eventcode und `eventConfidence === 1` den Outlook-Betreff `[Eventname] Originalbetreff`; lokale Zeile und Auditlog werden mitgeführt. Niedrigere Konfidenzen, unbekannte Treffer und Fehler bleiben unverändert. Outlook-Graph-Update, Idempotenz und die unsichere Schwelle sind durch Backend-Tests abgesichert; der Bestätigungsablauf durch einen Browser-Test.
